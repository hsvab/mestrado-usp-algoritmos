...
od <- od  %>%
  ...
  mutate(VIAG_MOTIVO_TRAB=ifelse(
    (MOTIVO_DEST==1 | MOTIVO_DEST==2 | MOTIVO_DEST==3) & !is.na(MOTIVO_DEST) & (SERV_PAS_DEST!=1)
    ,
    VIAG_MOTIVO_TRAB+1,
    VIAG_MOTIVO_TRAB
  )) 
...
od <- od  %>%
  ...
  mutate(VIAG_MOTIVO_EDUC=ifelse(
    (MOTIVO_DEST==4) & !is.na(MOTIVO_DEST) & (SERV_PAS_DEST!=1)
    ,
    VIAG_MOTIVO_EDUC+1,
    VIAG_MOTIVO_EDUC
  ))
...
od <- od  %>%
  ...
  mutate(VIAG_MOTIVO_RES=ifelse(
    (MOTIVO_DEST==8) & !is.na(MOTIVO_DEST) & (SERV_PAS_DEST!=1)
    ,
    VIAG_MOTIVO_RES+1,
    VIAG_MOTIVO_RES
  ))
...
od <- od  %>%
  ...
  mutate(VIAG_MOTIVO_MANUT_COMPRAS=ifelse(
    (MOTIVO_DEST==5 | MOTIVO_DEST==6) & !is.na(MOTIVO_DEST) & (SERV_PAS_DEST!=1)
    ,
...
od <- od  %>%
  ...
  mutate(VIAG_MOTIVO_LAZER_OUTROS=ifelse(
    (MOTIVO_DEST==7 | MOTIVO_DEST==9) & !is.na(MOTIVO_DEST) & (SERV_PAS_DEST!=1)
    ,
