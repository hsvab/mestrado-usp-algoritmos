od77_original <- fread('../../bases/bancos-separados/OD_1977_consolidado.csv',
                      sep=';', dec=',', header=TRUE,
                      colClasses = c(
                        'ID_PESS'='character',
                        'ID_VIAG'='character',
...
# od77 %>% filter(FE_FAM==55.53, round(REN_FAM,2)==round(1393.32/0.422345901,2), IDADE==19)
...
# od77 %>% filter(FE_FAM==48.75, round(REN_FAM,2)==round(9129.85/0.422345901,2), SEXO==0)
...
# od77_original %>% filter(FE_DOM==10, round(REN_IND,2)==round(451.91/0.422345901,2), IDADE==66)
# Atribuir 1 a F_DOM
od77[117516,'F_DOM'] = 1
...
od77 <- troca_linhas(od77, 29736)

# Trocar as duas linhas
od77 <- troca_linhas(od77, 111595)

# Trocar as duas linhas
od77 <- troca_linhas(od77, 180455, 180453)

# Trocar as duas linhas
od77 <- troca_linhas(od77, 210077)

od77 <- troca_linhas(od77, 79488)

## Corrigindo familias embaralhadas
## As trocas devem ocorrer na exata ordem que está abaixo
od77 <- troca_linhas(od77,79458 )
od77 <- troca_linhas(od77,79457 )
od77 <- troca_linhas(od77,79460 )
od77 <- troca_linhas(od77,79459 )
od77 <- troca_linhas(od77,79458 )
od77 <- troca_linhas(od77,79462 )
od77 <- troca_linhas(od77,79461 )
od77 <- troca_linhas(od77,79460 )
od77 <- troca_linhas(od77,79459 )
od77 <- troca_linhas(od77,79463 )
od77 <- troca_linhas(od77,79462 )
od77 <- troca_linhas(od77,79461 )
od77 <- troca_linhas(od77,79460 )
od77 <- troca_linhas(od77,79477 )
od77 <- troca_linhas(od77,79479 )
od77 <- troca_linhas(od77,79478 )
od77 <- troca_linhas(od77,79480 )
od77 <- troca_linhas(od77,79479 )
od77 <- troca_linhas(od77,79482 )
od77 <- troca_linhas(od77,79481 )
od77 <- troca_linhas(od77,79480 )
od77 <- troca_linhas(od77,79483 )
od77 <- troca_linhas(od77,79482 )
od77 <- troca_linhas(od77,79481 )
#
od77 <- troca_linhas(od77,79458 )
od77 <- troca_linhas(od77,79469 )
od77$ID_ANTES_DE_REMOVER <- row.names(od77)
######
## Linhas a serem removidas

# od77_original %>% filter(FE_DOM==27.41, round(REN_FAM,1)==round(1393.32/0.422345901,1))
# APAGAR todos os 5 registros
od77[21618:21622,'REMOVER'] = TRUE

# od77 %>% filter(FE_FAM==20.7, round(REN_FAM,2)==round(1882.82/0.422345901,2), COND_MORA==5, SUBZONA_DOM==88)
# Excluir todos os 6 registros
od77[111969:111974,'REMOVER'] = TRUE

# od77 %>% filter(FE_FAM==30.75, round(REN_FAM,2)==round(0/0.422345901,2))
# Excluir todos os 5 registros
od77[114451:114455,'REMOVER'] = TRUE

# od77 %>% filter(FE_FAM==1.18, round(REN_FAM,2)==round(0/0.422345901,2))
...
write.table(od77_consolidado_01, file='od_1977_consolidado_01.csv',
            row.names=FALSE, sep=';', dec=',')

# Passando consolidado_01 pelo ipython notebook

od77_processado_01 <- fread( 'od_1977_processado_01.csv',
                          sep=';', dec=',', header=TRUE,
                          colClasses = c(
                            'ID_PESS'='character',
                            'ID_VIAG'='character',
...
# ids.originais.com.problema <- od77_processado %>%
...
for( linha in casos.com.mais.de.uma.idade$ID_de_troca ) {
  od77_consolidado_02 <- troca_linhas(od77_consolidado_02, linha)
}

od77_consolidado_02$ID_CONSOLIDADO_02 <- row.names(od77_consolidado_02)

write.table(od77_consolidado_02, file='od_1977_consolidado_02.csv',
            row.names=FALSE, sep=';', dec=',')

# Passando consolidado_02 pelo ipython notebook

od77_processado_02 <- fread('od_1977_processado_02_v3.csv',
                          sep=';', dec=',', header=TRUE,
                          colClasses = c(
                            'ID_PESS'='character',
                            'ID_VIAG'='character',
...
all.equal(select(od77_processado_02,FE_DOM,IDADE,H_CHEG,F_PESS), select(od77_consolidado_02,FE_DOM,IDADE,H_CHEG,F_PESS))
