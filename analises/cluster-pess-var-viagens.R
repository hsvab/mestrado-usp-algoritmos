...
tempo <- system.time(pess_hcluster <- hclust.vector(od_cluster_pess, method="ward", metric="euclidean", p=NULL))
...
od <- left_join(od, select(od_cluster_pess, ID_PESS, pesscluster2_ward, pesscluster3_ward, pesscluster4_ward), by='ID_PESS')
...
# Incorporando as variáveis indicativas de cluster no BDU
od <- left_join(od, select(od_cluster_pess, ID_PESS, pesscluster2_centroide, pesscluster3_centroide, pesscluster4_centroide), by='ID_PESS')
